...
volatile long count_e1; // used by encoder to count the rotation (Left)
...
volatile long count_e0; // used by encoder to count the rotation (Right)
...
ISR( INT6_vect ) {
  ...
  switch ( state ) {
    ...
    default:             break;  // used in the case of: Invalid, or No movement.
  }
...
ISR( PCINT0_vect ) {
  ...
  // the direction.  However it illustrates well
  // against the lecture slides.  
  // !!! note that I can optimise this code using the default switch statement (bruno)
  // This is the Right encoder
  switch( state ) {
